...
class InitialSentence:
    ...
    def __check_args(self) -> None:
        if (
            ...
            or not isinstance(self.info, tuple)
            or len(self.info) != 2
            or not isinstance(self.info[0], str)
...
class Database:
    ...
    def __init__(
        self, db_name: str, user: str, host: str, port: str, password: str = "***"
    ) -> None:
        """
    ...
    def get_clubs_by_category(self, category_id: int) -> list | Any:
        ...
        """
        self.cursor.execute(
            "SELECT name, link, description FROM clubs WHERE category_id = %s", (category_id,)
        )
        return self.cursor.fetchall()
